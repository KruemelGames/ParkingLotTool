...
$ziel = Join-Path $PSScriptRoot '..\artifacts\drei'
...
foreach ($name in $namen) {
 ...
 if ($LASTEXITCODE -ne 0) { $fehlgeschlagen++ }
 $zeile = "$name Exitcode=$LASTEXITCODE"
 $zeilen += $zeile
 Add-Content (Join-Path $PSScriptRoot '..\bericht-drei.md') $zeile
 Write-Output $zeile
...
if ($fehlgeschlagen -ne 0) { throw "$fehlgeschlagen Pflichtlaeufe fehlgeschlagen" }
