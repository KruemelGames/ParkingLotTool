...
internal static partial class Program
{
    private static int PruefeBushaltestellen()
    {
        ...
        Check(BusStopSnap.TryFind(layout, new float2(20, 5), 8,
        ...
        Check(!BusStopSnap.TryFind(layout, new float2(20, 8.01f), 8,
        ...
        Check(!BusStopSnap.TryFind(ohneZoning, new float2(20, 4), 8,
        ...
        Check(BusStopSnap.TryFind(mitKreuzung, new float2(29, 5), 8, out var k1)
            && !AufKreuzung(k1) && math.abs(k1.Position.x - 22f) < 0.05f,
            "Kreuzung: Punkt springt vor die Querstrasse");
        Check(BusStopSnap.TryFind(mitKreuzung, new float2(33, -5), 8, out var k2)
            && !AufKreuzung(k2) && math.abs(k2.Position.x - 38f) < 0.05f,
            "Kreuzung: Punkt springt hinter die Querstrasse");
        Check(BusStopSnap.TryFind(mitKreuzung, new float2(51, 5), 8, out var k3)
            && !AufKreuzung(k3),
            "T-Stoss einer Fahrgasse wird ebenfalls uebersprungen");
        Check(BusStopSnap.TryFind(mitKreuzung, new float2(12, 5), 8, out var k4)
        ...
        for (var x = 0f; x <= 60f; x += 0.25f)
            foreach (var y in new[] { 5f, -5f })
                if (BusStopSnap.TryFind(mitKreuzung, new float2(x, y), 8, out var h))
                {
                    probe++;
        ...
        Check(BusStopSnap.TryFind(layout, new float2(14, 5), 8, out var g1,
                vorhanden, false)
            && g1.Left && math.abs(g1.Position.x - 10f) < 1e-3f,
            "gegenueber auf gleicher Hoehe eingerastet");
        Check(BusStopSnap.TryFind(layout, new float2(14, 5), 8, out var g2,
                vorhanden, true)
            && math.abs(g2.Position.x - 14f) < 1e-3f,
            "Shift: kein Einrasten");
        Check(BusStopSnap.TryFind(layout, new float2(14, -5), 8, out var g3,
                vorhanden, false)
            && math.abs(g3.Position.x - 14f) < 1e-3f,
            "gleiche Seite rastet nicht ein");
        Check(BusStopSnap.TryFind(layout, new float2(19, 5), 8, out var g4,
                vorhanden, false)
        ...
        Check(BusStopSnap.TryFind(layout, new float2(14, 5), 8, out var g5,
                umgedreht, false)
