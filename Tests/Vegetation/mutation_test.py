...
variants=[('Original',source),('Altersstufen',source.replace('mask &= 63;', 'mask &= 15;')),('Abstand',source.replace('if (!clear) continue;','')),('Dichte',source.replace('plan.Plants.RemoveAll(p=>Unit(Hash(p.Seed+97)) >= math.clamp(density,0,100)/100f);','')),('Median',source.replace('var world = origin + axis * p.x + across * p.y;','var world = origin + axis * p.x + across * (p.y + (options.Line ? .4f : 0));'))]
